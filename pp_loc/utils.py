...
class ReplayBuffer(object):
    def __init__(self, message_dim, n_agents, max_size=int(1e5)):
        self.max_size = max_size
        self.ptr = 0  # pointer to next position
        self.size = 0

        self.message_last_time = np.zeros((max_size, (message_dim + 5* n_agents) * n_agents))
        self.message_curr_time = np.zeros((max_size, message_dim * n_agents))

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    def add(self, message_last_time, message_curr_time):
        self.message_last_time[self.ptr] = message_last_time.detach().numpy()
        self.message_curr_time[self.ptr] = message_curr_time.detach().numpy()
    ...
    def sample(self, batch_size):
        ...
        return (torch.DoubleTensor(self.message_last_time[ind]).to(self.device),
                torch.DoubleTensor(self.message_curr_time[ind]).to(self.device))
