...
class GraphAttention(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, negative_slope, num_heads=1, bias=True, self_loop_type=2,
    ...
    def forward_single(self, input, adj):
        """
    ...
    def forward_batch(self, input, adj):

        N = input.size()[1]  # N=n_agents
        input = input.view(-1, self.in_features)  # input: (n_agents*n_agents, hid_size)
        h = torch.mm(input, self.W).view(N, N, self.num_heads, self.out_features)
        ...
        for head in range(self.num_heads):
            # coeff_i, coeff_j (tensors): intermediate matrices to calculate unnormalized coefficients [N * 1]
            coeff_i = torch.bmm(h[:, :, head, :],
                                self.a_i[head, :, :].expand(N, self.a_i.size()[1], self.a_i.size()[2]))
            coeff_j = torch.bmm(h[:, :, head, :],
                                self.a_j[head, :, :].expand(N, self.a_j.size()[1], self.a_j.size()[2]))
            # coeff (tensor): the matrix of unnormalized coefficients for each head [N * N * 1]
            coeff = coeff_i.expand(N, N, N) + coeff_j.transpose(1, 2).expand(N, N, N)
            coeff = coeff.unsqueeze(-1)
        ...
        adj = adj.unsqueeze(-1).expand(N, N, N, self.num_heads)
        attention = e * adj
        ...
        if self.normalize:
            if self.self_loop_type != 1:
                attention += 1e-15
            attention = attention / attention.sum(dim=2).unsqueeze(dim=2).expand(N, N, N, self.num_heads)
            attention = attention * adj
        # dropout on the coefficients
    ...
    def forward(self, input, adj, type='b'):
        """
        Forward function for the graph attention layer used in MAGIC
        ...
        if type == 'b':
            return self.forward_batch(input, adj)
        elif type == 's':
            return self.forward_single(input, adj)
        else:
            raise 'undefined forward type for gnn_layers'
