...
class MAGIC(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.fe = lambda epoch_idx: math.exp(-1. * epoch_idx / 10)
    ...
    def forward(self, x, memodel, t, epoch, info={}):
        ...
        cm_ = memodel(ma0)
        cm_ = cm_.view(self.nagents, self.nagents, self.message_size).detach()
        ...
        if t == 0 or rand <= e:
            ...
            self.curr_message = comm.repeat(self.nagents, 1, 1)
        else:
        ...
        comm = torch.diagonal(comm_curr).T
